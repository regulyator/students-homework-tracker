...
public class HomeWorkPullRequestsScannerImpl implements HomeWorkPullRequestsScanner {
    ...
    public void scanHomeWorksPullRequests() {
        var activeStudentGroups = dataServiceApiExchangeService.getActiveStudentGroups();
        var groupsIds = activeStudentGroups.stream()
                .map(studentGroupDto -> studentGroupDto.getGroup().getId())
                .collect(Collectors.toList());

        var activeHomeworks = dataServiceApiExchangeService.getAllActiveHomeWorksByGroupIds(groupsIds);
